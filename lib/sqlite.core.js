...
var { getSQLite } = require("../src/NativeSQLiteModule");
...
plugin.exec = function(method, options, success, error) {
  ...
  var native = getSQLite();
  if (!native) {
    throw new Error("react-native-sqlite-storage: native module 'SQLite' is not linked. Did you run pod install / rebuild the app?");
  }
  native[method](options, success, error);
};
...
SQLiteFactory.prototype.deleteDatabase = function(first,success, error) {
  ...
  plugin.exec("deleteDb",args,mysuccess,myerror);
};
...
module.exports.exec = plugin.exec;
